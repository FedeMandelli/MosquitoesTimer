...
def update_info(t_stamp, event):
    ...
    # update event log
    log_info = f'{ev_info[0]["event"]} - {ev_info[0]["duration"]}'
    log_lbl.txt = log_info if log_lbl.txt == '' else f'{log_lbl.txt}\n{log_info}'
    log_lbl.lbl.configure(text=log_lbl.txt)
